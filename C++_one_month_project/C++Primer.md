...
- [C++ Primer](#c-primer)
    ...
    - [Chapter 15 面向对象程序设计](#chapter-15-%E9%9D%A2%E5%90%91%E5%AF%B9%E8%B1%A1%E7%A8%8B%E5%BA%8F%E8%AE%BE%E8%AE%A1)
        ...
        - [派生类](#%E6%B4%BE%E7%94%9F%E7%B1%BB)
            ...
            - [防止继承](#%E9%98%B2%E6%AD%A2%E7%BB%A7%E6%89%BF)
            - [基类与派生类类型转换](#%E5%9F%BA%E7%B1%BB%E4%B8%8E%E6%B4%BE%E7%94%9F%E7%B1%BB%E7%B1%BB%E5%9E%8B%E8%BD%AC%E6%8D%A2)
        - [虚函数](#%E8%99%9A%E5%87%BD%E6%95%B0)
            - [回避虚函数的机制](#%E5%9B%9E%E9%81%BF%E8%99%9A%E5%87%BD%E6%95%B0%E7%9A%84%E6%9C%BA%E5%88%B6)
        - [访问控制与继承](#%E8%AE%BF%E9%97%AE%E6%8E%A7%E5%88%B6%E4%B8%8E%E7%BB%A7%E6%89%BF)
                - [受保护的成员](#%E5%8F%97%E4%BF%9D%E6%8A%A4%E7%9A%84%E6%88%90%E5%91%98)
                - [公有/私有/保护继承](#%E5%85%AC%E6%9C%89%E7%A7%81%E6%9C%89%E4%BF%9D%E6%8A%A4%E7%BB%A7%E6%89%BF)
...
如果基类定义了一个静态成员,则整个继承体系中只存在该成员的唯一定义.不论从基类中派生出来多少派生类,对于每个静态成员来说都只存在唯一实例.

静态成员遵循通用的访问控制规则:如果基类的成员是private的,则派生类无权访问他.假设某静态成员是可访问的,则我们既可以通过基类和派生类使用它.
...
```c++
class NoDerived final{};//不能用作基类
```

#### 基类与派生类类型转换

+ **不存在基类向派生类的隐式类型转换**.之所以存在派生类向基类的隐式类型转换,因为派生类都包含基类的一部分,而基类的引用和指针绑定到派生类的基类部分.
+ 派生类向基类的隐式类型转换只对**指针和引用**有效,在**对象**间不存在类型转换.当我们给基类的构造函数传递一个派生类对象时,实际运行的构造函数是基类的.

拷贝和赋值时造成切割(sliced down)问题.如何避免? 使用指针或引用.(Effective C++ item20)

```c++
Bulk_quote bulk;//派生类对象
Quote item(bulk);//使用基类的拷贝构造函数,会忽略派生类的成员部分.
item=bulk;//使用基类的拷贝赋值
```

### 虚函数

当我们使用基类的指针调用基类中定义的一个虚函数时,我们不知道该函数真正作用的对象是什么类型.直到运行时才会决定执行哪一个版本,判断的依据是指针或引用所绑定的对象的真是类型.

一个派生类的函数如果覆盖了某个继承而来的虚函数,则他的形参类型必须和其覆盖掉的函数完全一致.

#### 回避虚函数的机制

在某些情况下,我们希望对虚函数的调用不要进行动态绑定,强迫执行虚函数的某个版本.

```c++
double dis=baseP->Quote::net_price(42);//强行调用基类的虚函数
```
...
+ 和私有成员类似,受保护的成成员对于类的用户不可访问.
+ 和共有成员类似,受保护的成员对于派生类的成员函数和友元可访问.
+ 派生类的成员和友元只能通过派生类的对象来访问基类的受保护成员.派生类对于一个基类对象中的受保护成员,没有任何访问权限.
...
```
如果派生类(及其友元)能够访问基类对象的受保护成员.那么就造成了,这个函数不是基类的友元,但是却可以访问他的受保护和私有的成员.这样就可以通过派生一个类,规避掉protected提供的访问保护了.

**因此,派生类的成员和友元只能通过派生类的对象,访问其中的基类部分的受保护成员.对于普通基类对象的成员不具有特殊的访问权限.**
...
Pub_Derv d1;
Priv_Derv d2;
d1.pub_mem();//ok,pub_mem()在派生类中是public
d2.pub_mem();//error pub_mem()在派生类中是private
```
假设我们之前还定义了一个名为prot_Derv的类,他采用受保护继承,则Base的所有的公有成员在新定义的类中都是受保护的.prot_Derv的用户不能访问pub_mem,但是prot_Derv的成员和友元可以访问那些继承而来的成员.
...
当我们delete一个动态分配的对象的指针时将执行析构函数.例如,如果我们delete一个`Quote*`类型的指针,则该指针有可能实际指向了一个`Bulk_quote`类型的对象.如果这样的话,编译器就应该清楚他执行的是`Bulk_quote`的析构函数.和其他函数一样,我们通过基类中将析构函数定义成虚函数以确保执行正确的析构函数版本:
...
和其他函数一样,析构函数的虚属性也会被继承.因此,无论Quote的派生类使用合成的析构函数还是定义自己的析构函数,都将是虚析构函数.只要基类的析构函数是虚函数,就能确保当我们delete基类指针时将运行正确的析构函数版本.
...
delete itemP;//调用派生析构
```
> 如果基类析构不是虚函数,则delete一个指向派生对象的基类指针将会产生未定义的行为!

之前我们介绍过一条经验,如果一个类需要析构函数,那么他同时也需要拷贝和赋值操作(P447).**基类的析构函数并不遵循以上原则**.他是一个重要的例外.

如果一个类定义了虚析构函数,编译器将会**阻止合成移动操作**.

Q15.24:什么类需要虚析构函数?虚析构函数都需要执行什么操作?

基类;如果没有指针类成员,则可以使用合成版本析构函数;如果有指针成员,需要自定义析构函数对指针成员进行清除.
